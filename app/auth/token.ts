...
const TOKEN_SETTINGS:{} = {expiresIn: '10s'}
...
const Token = {
    ...
    refreshToken: function(refreshToken:string):tokenResponse | Error {
        ...
        const response = jwt.verify(refreshToken, process.env.REFRESH_GEN_KEY, async (err:Error, user:userEncodedInfo) => {
            const userInfo:userEncodedInfo = {
                userId: user.userId,
                email: user.email,
                username: user.username
            }
            if (await getRefreshDB(userInfo.userId, refreshToken)) {
